...
- `id` — a permanent type prefix followed by four digits (for example, `C1001`)
- `variant` — distinguishes multiple mods that use the same map name
- `description` — a short explanation
- `categories` — an array of every filter where the entry should appear
...
Number IDs sequentially within their entry type. Categories are deliberately not encoded in the ID, so changing an entry's categories never requires renaming it or its image:

- `C` — map Configs
- `T` — Tweaks
- `M` — Mods

For example, a config that belongs in two filters uses:
...
1. Add the screenshot to the repository's `images/maps` folder.
2. Name it after the config ID using an uppercase prefix and lowercase extension, such as `C1001.png`.
3. Commit and push the image. The matching card will display it automatically.
...
Animated GIFs work directly in tweak cards. Add them under `images/tweaks` and use a path such as:
